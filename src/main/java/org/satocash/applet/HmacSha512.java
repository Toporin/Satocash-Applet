...
public class HmacSha512 {
    ...
    public static short computeHmacSha512(byte[] key, short key_offset, short key_length, 
        ...
        if (key_length>BLOCKSIZE || key_length<0){
            ISOException.throwIt(Satocash.SW_HMAC_UNSUPPORTED_KEYSIZE); // don't accept keys bigger than block size
        }
        if (message_length>HASHSIZE || message_length<0){
            ISOException.throwIt(Satocash.SW_HMAC_UNSUPPORTED_MSGSIZE); // don't accept message bigger than block size (should be sufficient for BIP32)
        }
